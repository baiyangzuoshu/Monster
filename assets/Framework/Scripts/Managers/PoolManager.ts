...
export class PoolManager extends Component {
    ...
    private AddNodePool(poolName: string, nodeOrPrefab: Node | Prefab, count: number = 1) {
    ...
    private PutNode(poolName: string, node: Node): void {
        if(!this.nodePools[poolName]) {
            return;
        }
    ...
    private GetNodeInPool(poolName: string): Node {
        if(!this.nodePools[poolName]) {
            return null;
        }
